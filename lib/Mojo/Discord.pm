...
sub move_channel_category {
    ...
    $self->rest->move_channel_category($channel, $guild, $callback);
}

sub create_voice_channel {
    my ($self, $guild, $channel, $parent, $callback) = @_;

    $self->rest->create_voice_channel($guild, $channel, $parent, $callback);
...
sub create_text_channel {
    ...
    $self->rest->create_text_channel($guild, $parent, $channel, $topic, $callback);
...
sub delete_text_channel {
    ...
    $self->rest->delete_text_channel($channel, $callback);
}
...
sub send_file
{
    ...
    unless ( defined $args->{'type'} and $args->{'type'} =~ /^gifv|image|video|docx$/i )
    {
