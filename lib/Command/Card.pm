...
sub cmd_card
{
    ...
    if ( $deck->size )
    {
        ...
        if ( $args =~ /battle (.*)/i ) {
            my $b = $1;
            $self->discord->send_image($channel_id, {'content' => '<@' . $author_id  . '>', 'name' => $name, 'path' => $image});
            my $p1 = cValue($card_code);
            $deck->give_cards($hand, 1);
            $self->decks->{$channel_id} = $deck;
            $self->hands->{$channel_id}{$author_id} = $hand;
            $card_code = $hand->{'cards'}[-1]->print("short"); $card_code =~ s/\s//g;
            $card_name = $hand->{'cards'}[-1]->print("long");
            $image = "lib/Command/Card/images/$card_code.png";
            $name = "$card_code.png";
            my $p2 = cValue($card_code);
            $self->discord->send_image($channel_id, {'content' => $b, 'name' => $name, 'path' => $image});
            ...
            #$self->discord->send_image($channel_id, {'description' => 'test', 'content' => 'test2', 'name' => $name, 'path' => $image});
        }
    }
    else
    {
...
sub cValue {
    my $c = shift;
    my ($value, $suit) = split //, $c;

    return $c !~ /^\d/ ? $trumps{$value} + $suits{$suit} : $value + $suits{$suit};
}
